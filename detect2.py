...
def run(
    ...
    fsrcnn_scale=3,  # fsrcnn super-resolution scale factor
    fsrcnn_weight=3,  # fsrcnn weights
):
    source = str(source)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:
                        ...
                        # # Combine the equalized image with the original image
                        result = cv2.addWeighted(imc, 0.5, eq_color, 0.7, 0)
                        ...
                        savePath = increment_path(save_dir / "crops" / f"{p.stem}.jpg")
                        print("Saving crop to: ", savePath)
                        save_one_box(xyxy, result, file=savePath, BGR=True)

                        if conf <= 0.75:
                            from FSRCNN import test

                            test.test(
                                weight=str(fsrcnn_weight),
                                image_file=str(savePath),
                                scale=int(fsrcnn_scale),
                                save_dir=save_dir / "FSRCNN",
                            )
                            incName = str(savePath).split("\\")[-1].split(".")[0]
                            print("incName: ", incName)
                            newImagePath = (
                                save_dir
                                / "FSRCNN"
                                / f"{incName}_fsrcnn_x{fsrcnn_scale}.jpg"
                            )
                            ...
                            detect.run(
                                ...
                                pre_conf=conf,
                                weights=weights,
                                savePath=str(save_dir / "improved"),
                                fileName=f"{p.stem}",
                            )
...
def parse_opt():
    ...
    parser.add_argument("--fsrcnn-scale", type=int, default=3, help="fsrcnn scale")
    parser.add_argument("--fsrcnn-weight", type=int, default=3, help="fsrcnn weight")
